...
namespace MinecraftClient.Mapping.BlockPalettes
{
    public class Palette116 : BlockPalette
    {
        ...
        public override bool IsWaterLogged(int targetId)
        {
            if (!propsInit) ReadDataAndInit();

            return waterLoggedStates.Contains(targetId);
        }

        private static Dictionary<int, Dictionary<string, string>> blocPropsTable = new Dictionary<int, Dictionary<string, string>>();
        // Those blockstates have a true waterlogged property attached to
        private static List<int> waterLoggedStates = new List<int>();
        private static bool propsInit = false, refreshing = false;

        public override Dictionary<string, string> GetProps(int targetId)
        {
            if (!propsInit) ReadDataAndInit();

            if (blocPropsTable.ContainsKey(targetId))
                return blocPropsTable[targetId];
            
            return null;
        }

        private void ReadDataAndInit()
        {
            ...
            Json.JSONData palette = Json.ParseJson(File.ReadAllText(Application.streamingAssetsPath + "/JsonAssets/1165/blocks.json", Encoding.UTF8)); // TODO
            foreach (KeyValuePair<string, Json.JSONData> item in palette.Properties)
            {
                ...
                foreach (Json.JSONData state in item.Value.Properties["states"].DataArray)
                {
                    ...
                    bool IsWeirdWaterBlock = weirdWaterBlocks.Contains(item.Key);

                    if (state.Properties.ContainsKey("properties"))
                    {
                        // This block state contains block properties
                        blocPropsTable[id] = new Dictionary<string, string>();
                        foreach (var prop in state.Properties["properties"].Properties)
                        {
                            blocPropsTable[id].Add(prop.Key, prop.Value.StringValue);
                            // Store interesting properties for later use...
                            if (IsWeirdWaterBlock || (prop.Key == "waterlogged" && prop.Value.StringValue == "true"))
                            {
